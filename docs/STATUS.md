...
| **10** | **Firebase Stack & Branded QR Ecosystem** | Firebase Auth (Email/Google) + Cloud Firestore real-time sync, offline fallback, branded center-logo QRs, permanent FayrouzPass Member QR, and in-store table tent detection (`?venue=almond`). | ✅ **Completed** |
| **11** | **FayrouzPass™ ID Evolution & Counter Fast-Recall** | Standardized format to `JO-XXXXX` (100k capacity per region), dedicated counter recall UI with locked country badge `[ 🇯🇴 JO ]`, 5-digit numeric keypad (`inputMode="numeric"`), and auto-normalizing Firestore lookup. | ✅ **Completed** |
